...
set -e
stage=0
aug_list="reverb noise clean"  #clean refers to the original train dir
use_ivectors=true
num_reverb_copies=1

# Alignment directories
lda_mllt_ali=tri2b_ali_si284
clean_ali=tri4b_ali_train_si284
...
if [ $stage -le 2 ]; then
  ...
  if [[ $aug_list =~ "clean" ]]; then
    ln -sf data/${train_set}_aug data/${train_set}_aug_clean
  else
    combine_str+="data/$train_set "
...
if [ $stage -le 2 ]; then
  ...
  if [[ $aug_list =~ "clean" ]]; then maybe_clean=${train_set}_aug_clean; else maybe_clean= ; fi
...
if [ $stage -le 3 ] && $generate_alignments; then
  ...
  if [[ $aug_list =~ "clean" ]]; then
    ...
    steps/copy_ali_dir.sh --nj 40 --cmd "$train_cmd" \
      --include-original "$include_original" --prefixes "$prefixes" \
      data/${train_set}_aug_clean exp/${clean_ali}_clean exp/${clean_ali}_aug_clean
  fi
fi

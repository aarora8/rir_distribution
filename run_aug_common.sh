...
set -e
stage=0
aug_list="reverb clean"  #clean refers to the original train dir
use_ivectors=false
num_reverb_copies=1

# Alignment directories
lda_mllt_ali=tri2_ali_100k_nodup
clean_ali=tri3_ali_nodup
...
if [ $stage -le 1 ]; then
  ...
  if [[ $aug_list =~ "clean" ]]; then
    ln -sf ${train_set}_aug data/${train_set}_aug_clean
  else
    combine_str+="data/$train_set "
    utils/combine_data.sh data/${train_set}_aug_clean $combine_str
  fi
fi
...
if [ $stage -le 2 ]; then
  ...
  if [[ $aug_list =~ "clean" ]]; then maybe_clean=; else maybe_clean=${train_set}_aug_clean ; fi
  for dataset in ${train_set}_aug $maybe_clean; do
...
if [ $stage -le 4 ]; then
  ...
  if [[ $aug_list =~ "clean" ]]; then
    ln -sf ${train_set}_aug_hires data/${train_set}_aug_hires_clean
  else
    mv data/${train_set}_aug_clean_hires data/${train_set}_aug_hires_clean
  fi
fi
